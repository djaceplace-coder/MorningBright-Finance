-- FULL SUPABASE SQL CONSOLE PATCH --
-- This immediately permanently fixes all Admin RLS policies by using JWT instead of checking tables.

-- 1. Ensure support account is registered internally as admin
UPDATE public.users 
SET is_admin = true 
WHERE email = 'support@morningbrightfinance.com';
...
BEGIN
    ...
    -- Grant explicit true admin to the support email
    IF current_email = 'support@morningbrightfinance.com' THEN
        RETURN true;
    END IF;
    RETURN false;
END;
